...
import type { IAuthLoginRes, ILoginRes, IUserInfoRes } from '@/api/types/login'
...
const DEFAULT_ACCESS_EXPIRES_IN = 60 * 60 * 24 * 7
const DEFAULT_REFRESH_EXPIRES_IN = 60 * 60 * 24 * 30
...
function normalizeTokenInfo(loginRes: ILoginRes): IAuthLoginRes {
  const accessToken = loginRes.accessToken || loginRes.access_token
  const refreshToken = loginRes.refreshToken || loginRes.refresh_token
  const accessExpiresIn = loginRes.accessExpiresIn || loginRes.expiresIn || loginRes.expires_in
  const refreshExpiresIn = loginRes.refreshExpiresIn

  if (accessToken) {
    return {
      accessToken,
      refreshToken,
      accessExpiresIn: accessExpiresIn || DEFAULT_ACCESS_EXPIRES_IN,
      refreshExpiresIn: refreshExpiresIn || DEFAULT_REFRESH_EXPIRES_IN,
    }
  }
  ...
  if (loginRes.token) {
    return {
      ...
      expiresIn: loginRes.expiresIn || DEFAULT_ACCESS_EXPIRES_IN,
    }
  }

  throw new Error('登录接口未返回可用 token')
}

function getLoginUserInfo(loginRes: ILoginRes): IUserInfoRes | undefined {
  return loginRes.userInfo || loginRes.user
}
  ...
  () => {
    ...
    const setTokenInfo = (val: IAuthLoginRes) => {
      ...
      if (isSingleTokenRes(val)) {
        ...
        if (val.refreshToken && val.refreshExpiresIn) {
     ...
     */
    async function _postLogin(tokenInfo: IAuthLoginRes, loginUserInfo?: IUserInfoRes) {
    ...
    const login = async (loginForm: ILoginForm) => {
      try {
        ...
        const tokenData = normalizeTokenInfo(res)
        const userInfo = getLoginUserInfo(res)
        await _postLogin(tokenData, userInfo)
        uni.showToast({
          title: '登录成功',
          icon: 'success',
        })
    ...
    const refreshToken = async () => {
      ...
      try {
        ...
        const normalizedToken = normalizeTokenInfo(res)
        setTokenInfo(normalizedToken)
